...
When the user clicks "Scan This Site", SecureCheck may process the following data locally in the browser:
...
SecureCheck uses this local data only to:
...
SecureCheck keeps scan results in extension memory only during the extension session. It does not persist scan results in Chrome storage.

The extension may store the selected UI language locally so the popup can remember the user's preference.

## Network Requests

SecureCheck sends network requests only to the website being scanned. These requests are used for passive checks such as reading response headers, checking same-origin JavaScript bundles, and testing whether common sensitive paths are publicly reachable.

SecureCheck does not send scan results, URLs, page content, cookies, or findings to any external analytics service, AI service, advertising network, or developer-operated server.

## Permissions

SecureCheck requests only the permissions needed for its single purpose:
...
SecureCheck's use and transfer of information received from Chrome APIs is limited to providing the extension's single purpose. SecureCheck does not sell user data, use user data for advertising, or use user data to determine creditworthiness or for lending purposes.

## Security

SecureCheck masks detected secret-like values before showing them in the UI. It avoids reading cookie values and avoids persistent storage of scan findings.
...
For privacy or security questions, open an issue in the SecureCheck GitHub repository:
